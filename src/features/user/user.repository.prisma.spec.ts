...
function makeRepository() {
  ...
  const repository = new PrismaUserRepository(prismaClient as never);
  return { prismaClient, repository };
}

describe('PrismaUserRepository', () => {
  describe('Create records in the repository', () => {
    let prismaClient: { user: { create: ReturnType<typeof vi.fn> } };
    let repository: PrismaUserRepository;
    beforeEach(() => {
      const { prismaClient: client, repository: rep } = makeRepository();
      prismaClient = client;
      repository = rep;
    });
  ...
  describe('Read records from the repository', () => {
    let prismaClient: { user: { findUnique: ReturnType<typeof vi.fn> } };
    let repository: PrismaUserRepository;
    beforeEach(() => {
      const { prismaClient: client, repository: rep } = makeRepository();
      prismaClient = client;
  ...
  describe('Removing records from the repository', () => {
    let prismaClient: { user: { delete: ReturnType<typeof vi.fn> } };
    let repository: PrismaUserRepository;
    beforeEach(() => {
      const { prismaClient: client, repository: rep } = makeRepository();
      prismaClient = client;
